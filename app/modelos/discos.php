<?php
	class Discos extends Modelo {
		...
		function datosPorId($id) {
			...
			$sentencia = "SELECT disco_nombre, disco_fecha, disco_genero, (SELECT banda_nombre "
				."FROM bandas WHERE banda_id = d.banda_id) as banda FROM discos AS d WHERE disco_id = :id";
			$query = $con->prepare($sentencia);
			$query->execute([":id" => $id]);
			while($fila = $query->fetch()) {
				return [
					"id" => $id, "nombre" => $fila['disco_nombre'], "fecha" => date("d/m/Y", strtotime($fila['disco_fecha'])),
					"genero" => $fila['disco_genero'],"artista" => $fila['banda'], "canciones" => $this->obtenerCanciones($id)
				];
			}
		}
		function obtenerCanciones($id) {
			$con = $this->datai::conectar();
			$parametrosEjecucion = [];
			$sentencia = "SELECT cancion_nombre, cancion_duracion FROM canciones WHERE disco_id = :id ORDER BY cancion_num";
			$query = $con->prepare($sentencia);
			$query->execute([":id" => $id]);
			$items = [];
			while($fila = $query->fetch()) {
				array_push($items, [
					"nombre" => $fila['cancion_nombre'], "duracion" => $fila['cancion_duracion']
				]);
			}
			return $items;
		}
